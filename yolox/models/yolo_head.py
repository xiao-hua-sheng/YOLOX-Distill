...
class YOLOXHead(nn.Module):
    def __init__(
        ...
        self.iou_loss = IOUloss(reduction="none",loss_type="eiou")
    ...
    def forward(self, xin, labels=None, imgs=None, T_pre=None):
        outputs = []
        origin_preds = []
        x_shifts = []
        y_shifts = []
    ...
    def distill_loss(self,model_pre, T_model_pre, T, dist_loss=None):
        t_ft = torch.cuda.FloatTensor if T_model_pre[0].is_cuda else torch.Tensor
        t_lcls, t_lbox, t_lobj = t_ft([0]), t_ft([0]), t_ft([0])

        DboxLoss = torch.nn.MSELoss(reduction="none")
        if dist_loss == "l2":
            DclsLoss = torch.nn.MSELoss(reduction="none")
        elif dist_loss == "kl":
            DclsLoss = torch.nn.KLDivLoss(reduction="none")
        else:
            DclsLoss = torch.nn.BCEWithLogitsLoss(reduction="none")
        DobjLoss = torch.nn.MSELoss(reduction="none")

        for i, pi in enumerate(model_pre):
            t_pi = T_model_pre[i]
            ...
            #cls = 5
            c_obj_scale = t_obj_scale.unsqueeze(-1).repeat(1,1,1,1,5)
            # if dist_loss == "kl":
    ...
    def get_losses(
        ...
        loss = reg_weight * loss_iou + loss_obj + loss_cls + loss_l1 + d_loss
        return (
            loss,
            reg_weight * loss_iou,
            loss_obj,
